...
class TestConsole implements Console {
  ...
  public assert(value?: any, message?: any, ...optionalParams: any[]): void {
    throw new Error("Method not implemented.");
  }
...
export class ConsoleLoggerImplementationTest {
  ...
  public async checkArgumentPassing(): Promise<void> {
    ...
    td.verify(
      consoleMock.log(td.matchers.isA(String), "ROOT", "", "Test Message 8"),
      { ignoreExtraArgs: false, times: 1 },
    );
  }

  @test
  public async checkGetImplementation(): Promise<void> {
    const c: Console = new TestConsole();
    const consoleMock = td.object(c);
    const logger = new ConsoleLoggerImplementation(consoleMock);
