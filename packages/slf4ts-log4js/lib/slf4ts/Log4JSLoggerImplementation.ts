 ...
 */
export class Log4JSLoggerImplementation
  implements LoggerImplementation<Logger, [string]>
{
  private readonly loggers = new Map<string, Logger>();
  private builder: LoggerBuilder<Logger, [string]> = (category: string) =>
    getLogger(category);
  ...
  public async log(...args: any[]): Promise<any> {
    ...
    const callArguments: any[] = [LogLevelMapping[level]].concat(
      ...additionalArguments,
    );
