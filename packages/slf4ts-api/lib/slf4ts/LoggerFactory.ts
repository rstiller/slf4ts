 ...
 */
export class DefaultLoggerInstance<T, P extends any[]>
  implements ILoggerInstance<T>
{
...
export class LoggerFactory {
  ...
  public static getLogger<T, P extends any[]>(
    ...
    if (LoggerFactory.LOGGER_INSTANCE_CACHE.has(compoundKey)) {
      return LoggerFactory.LOGGER_INSTANCE_CACHE.get(
        compoundKey,
      ) as ILoggerInstance<T>;
    }
  ...
  private static initialize(): void {
    ...
    if (BINDINGS.length > 1) {
      ...
      LoggerFactory.ROOT_LOGGER.info(message).catch(console.error);
    }
  ...
  private static logLevelChanged(event: any): void {
    ...
    LoggerFactory.LOGGER_INSTANCE_CACHE.forEach((logger, compoundKey) => {
      ...
      if (
        (groupEmpty && nameEmpty) ||
        (groupMatches && nameEmpty) ||
  ...
  private static configChanged(event: any): void {
    ...
    LoggerFactory.LOGGER_INSTANCE_CACHE.forEach((logger, compoundKey) => {
      const hasNameConfig = LoggerConfiguration.hasConfig(
        logger.getGroup(),
        logger.getName(),
      );
      ...
      if (
        (groupEmpty && nameEmpty && !hasGroupConfig && !hasNameConfig) ||
        (nameEmpty && groupMatches && !hasNameConfig) ||
        (groupMatches && nameMatches)
      ) {
        logger
          .getImpl()
          .setConfig(event.config, logger.getGroup(), logger.getName());
      }
    });
  }
}
