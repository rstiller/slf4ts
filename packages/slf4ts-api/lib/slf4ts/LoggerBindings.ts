...
export class LoggerBindings {
  ...
  public constructor(additionalPaths: string[] = []) {
    ...
    if ("mainModule" in process && "paths" in mainModule) {
      mainModule.paths.forEach((mainPath) => {
        additionalPaths.push(mainPath);
      });
    }
  ...
  private getAllModuleFolders(additionalPaths: string[]): string[] {
    ...
    rootPaths
      ...
      .forEach((rootPath) => {
        this.visitNodeModules(rootPath).forEach((folder) => {
  ...
  private visitNodeModules(rootPath: string): string[] {
    ...
    if (fs.existsSync(rootPath)) {
      ...
      files.forEach((folder) => {
        ...
        this.visitNodeModules(path.join(absolutePath, "node_modules")).forEach(
          (subfolder) => {
            moduleFolders.push(subfolder);
          },
        );
      });
    }
