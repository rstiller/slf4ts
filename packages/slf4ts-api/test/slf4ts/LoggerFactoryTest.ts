...
export class LoggerFactoryTest {
  ...
  public checkRootLogger(): void {
    ...
    const loggerImpl = (
      logger as DefaultLoggerInstance<any, any[]>
    ).getImpl() as any;
    logger.setMetadata({ mode: "Test" });

    expect(loggerImpl).to.exist;

    logger.info("Test Message").catch(fail);
  ...
  public checkGetNamedLogger(): void {
    ...
    expect(LoggerFactory.getLogger()).to.equal(LoggerFactory.getLogger());
    expect(LoggerFactory.getLogger(group)).to.equal(
      LoggerFactory.getLogger(group),
    );
    expect(LoggerFactory.getLogger(group, name)).to.equal(
      LoggerFactory.getLogger(group, name),
    );
    expect(LoggerFactory.getLogger(group)).to.not.equal(
      LoggerFactory.getLogger(group, name),
    );
    expect(LoggerFactory.getLogger()).to.not.equal(
      LoggerFactory.getLogger(group),
    );
    expect(LoggerFactory.getLogger()).to.not.equal(
      LoggerFactory.getLogger(group, name),
  ...
  public checkSetLogLevel(): void {
    ...
    const loggerImpl = (
      rootLogger as DefaultLoggerInstance<any, any[]>
    ).getImpl() as any;
  ...
  public checkCorrectLogLevelInvocations(): void {
    ...
    expect(logger.getLogLevel()).to.equal(LogLevel.INFO);
    logger.info("Test Message 1").catch(fail);
    ...
    logger.info("Test Message 2").catch(fail);

    expect(loggerImpl.calls).to.have.length(1);

    LoggerConfiguration.setLogLevel(LogLevel.DEBUG);
    expect(logger.getLogLevel()).to.equal(LogLevel.DEBUG);
    logger.info("Test Message 3").catch(fail);
  ...
  public checkMetadataArguments(): void {
    ...
    logger.info("Test Message 2", { arg1: "value1" }).catch(fail);
    expect(loggerImpl.calls).to.have.length(2);
    ...
    expect(loggerImpl.calls[2][4]).to.deep.equal({
      mode: "Test",
      version: "1.0.0",
    });

    logger.info("Test Message 4", { arg1: "value1" }).catch(fail);
    ...
    expect(loggerImpl.calls[3][5]).to.deep.equal({
      mode: "Test",
      version: "1.0.0",
    });

    const error = new Error("Test");
    logger.info("Test Message 5", { arg1: "value1" }, error).catch(fail);
    ...
    expect(loggerImpl.calls[4][6]).to.deep.equal({
      mode: "Test",
      version: "1.0.0",
    });

    logger.info("Test Message 6", null, error).catch(fail);
    ...
    expect(loggerImpl.calls[5][6]).to.deep.equal({
      mode: "Test",
      version: "1.0.0",
    });

    logger.info("Test Message 7", error).catch(fail);
    ...
    expect(loggerImpl.calls[6][5]).to.deep.equal({
      mode: "Test",
      version: "1.0.0",
    });
  }
  ...
  public checkSetMetadataOnLoggerFactory(): void {
    ...
    logger.info("Test Message 4").catch(fail);
    expect(loggerImpl.calls).to.have.length(4);
    ...
    logger.info("Test Message 5").catch(fail);
    expect(loggerImpl.calls).to.have.length(5);
  ...
  public checkSetMetadata(): void {
    ...
    expect(loggerImpl.setMetadataCalls[1][0]).to.deep.equal(
      namedLogger1Metadata,
    );
    ...
    expect(loggerImpl.setMetadataCalls[2][0]).to.deep.equal(
      groupLoggerMetadata,
    );
